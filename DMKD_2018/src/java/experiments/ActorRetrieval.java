...
public class ActorRetrieval{

public static void main(String[] args)
{
        HashMap<Integer,String> id2names = new HashMap<Integer,String>();

	ActorRetrievalCommandLineOptions options = new ActorRetrievalCommandLineOptions(args);
	...
	try{
	    ...
	    //hasher.normalizer.updateStatistics(train_data.values());
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(options.id2namesfile))) {
 	        String line;
  		...
  		for (Iterator<Integer> it=test_data.keySet().iterator();it.hasNext();)
      {
		      ...
		      System.out.print("test actor " + id2names.get(nexttestid) + "  neighbors: ");

		      for (int i=0;i<neighbors.size()-1;i++)
          {
			       System.out.print(id2names.get(neighbors.elementAt(i)) + ", ");
          }
			       System.out.print(id2names.get(neighbors.elementAt(neighbors.size()-1)) + "\n");
		}
    estimatedTime = (System.currentTimeMillis() - startTime)/1000;
...
class ActorRetrievalCommandLineOptions {
  ...
  public ActorRetrievalCommandLineOptions(String[] args)
  {
  ...
  {
    System.out.println("Usage:\n\ttet.rnn.ActorRetrieval [options] <database> <login> <password> <tetfile> <trainfile> <testfile> <id2namesfile> <outfile>");
