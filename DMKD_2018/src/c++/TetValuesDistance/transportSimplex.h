...
public:
	...
	long double *weights;						// Pointer to the weights of the features 
	TsSignature(int nin, TF *fin, long double * win):n(nin), features(fin), weights(win){};
};
...
typedef struct TsFlow {
	...
	long double amount;							// Amount of flow from signature1.features[from] to signature2.features[to]
} TsFlow;
...
typedef struct TsVogPen {
  ...
  int one, two;
  long double oneCost, twoCost;
} TsVogPen;

/* DECLARATION OF GLOBALS */
//long double ** _tsC = NULL;				// Cost matrix
long double _tsC[500][500];				// Cost matrix
long double _tsMaxC;						// Maximum of all costs
long double _tsMaxW;						// Maximum of all weights


/* INTERNAL FUNCTIONS */
long double _pivot(TsBasic * basics, TsBasic ** srcBasics, TsBasic ** snkBasics, bool ** isBasic, int n1, int n2);
TsStone * _BFS(TsStone *stoneTree, TsBasic ** srcBasics, TsBasic ** snkBasics, bool complete = false);
void _initVogel(long double *S, long double *D, TsBasic * basicsEnd, TsBasic ** srcBasics, TsBasic ** snkBasics, bool ** isBasic, int n1, int n2);
void _initRussel(long double *S, long double *D, TsBasic * basicsEnd, TsBasic ** srcBasics, TsBasic ** snkBasics, bool ** isBasic, int n1, int n2);
...
*/
template <class TF>
long double transportSimplex(TsSignature<TF> *signature1, TsSignature<TF> *signature2, long double (*grndDist)(TF *, TF *),
	...
	long double srcSum, snkSum, diff;  
	...
	long double * src = NULL;						//Array of source supplies
	long double * snk =NULL;							//Array of sink demands
	...
	try {
		...
		_tsMaxC = 0;
/*
		_tsC = new long double*[n1];
		for(i=0; i < n1; i++) 
			_tsC[i] = NULL;
*/
		for(i=0, P1=signature1->features; i < n1; i++, P1++) {
			//_tsC[i] = new long double[n2];					//What happens if bad  here?
			for(j=0, P2=signature2->features; j < n2; j++, P2++) {
				...
				long double d = grndDist(P1,P2);
				//std::cout << i << " " << j << " P1 " << *P1 << " P2 " << *P2 << " " << d << std::endl;
	         		_tsC[i][j] = d;
					if (_tsC[i][j] < 0) {
						std::cout << "Bad Input " << _tsC[i][j] << std::endl;
						throw TsErrBadInput;
					}
				}
		...
		src = new long double[n1];					//init the source array
		for (i = 0; i < signature1->n; i++) {
			src[i] = signature1->weights[i];
			if (src[i] < 0) {
				std::cout << "Bad Input src " << src[i] << std::endl;
				throw TsErrBadInput;
			}
		}
		if (n1 != signature1->n)
		...
		snk = new long double[n2];					//init the sink array
		for (i = 0; i < signature2->n; i++) {
			snk[i] = signature2->weights[i];
			if (snk[i] < 0) {
				std::cout << "Bad Input snk " << snk[i] << std::endl;
				throw TsErrBadInput;
			}
		}
		if (n2 != signature2->n)
...
long double _pivot(TsBasic * basics, TsBasic ** srcBasics, TsBasic ** snkBasics, bool ** isBasic, int n1, int n2)  {
	...
	long double * srcDuals = NULL;
	long double * snkDuals = NULL;
	...
	int i=0,j=0, lowI, lowJ;
	long double objectiveValue = TSINFINITY, oldObjectiveValue = 0;
	long double lowVal;
	int numPivots = 0;

	try {
		srcDuals = new long double[n1];
		snkDuals = new long double[n2];
	...
	while (1) {
		...
		for (XP = basics; XP != basicsEnd; XP++){
			...
			{
/*
				std::cout << "XP " << XP->i << " " << XP->j << " " 
				 << _tsC[XP->i][XP->j] << " " << XP->val << std::endl;
*/
		...
		// Marco HACK to solve a long double rounding problem
		if (lowVal<1e-30) lowVal=0;
/*
		std::cout << numPivots << " " << objectiveValue << " " << oldObjectiveValue - objectiveValue 
			<< " " << lowVal << " " << TSEPSILON * _tsMaxC << std::endl;
*/
		...
		if (lowVal >= -TSEPSILON * _tsMaxC || (oldObjectiveValue - objectiveValue) < TSPIVOTLIMIT) {
			...
			delete[] snkDuals;
			delete[] stonePath;
			//std::cout << numPivots << "\t" << "Total cost = " << objectiveValue << std::endl;
...
inline void addPenalty(TsVogPen * pitr, long double cost, int i) {
...
void _initRussel(long double *S, long double *D, TsBasic * basicsEnd, TsBasic ** srcBasics, TsBasic ** snkBasics, bool ** isBasic, int n1, int n2) {
	...
	TsRusPen *Uitr, *Vitr;
	long double cost, lowVal;
	
	try {
		Delta = new long double*[n1];
		for (i = 0; i < n1; i++)
			Delta[i] = new long double[n2];
	...
	for (i = 0; i < n1; i++) 
		for (j = 0; j < n2; j++) 
		{
			//std::cout << i << " " << j << " " <<_tsC[i][j] << std::endl;
	...
	while (Uhead != NULL && Vhead != NULL) {
		...
		basicsEnd->i = lowI;
		basicsEnd->j = lowJ;
		//std::cout<<"Russel "<<lowI<<" "<<lowJ<<" "<<lowVal<<std::endl;
